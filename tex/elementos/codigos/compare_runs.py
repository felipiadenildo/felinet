# compare_runs.py
# Gera o relatório comparativo de dois runs do pipeline (principal vs baseline)
# consolidando as métricas das Fases II, III e IV em uma tabela única, no
# formato esperado pela Tabela 6 do Capítulo 4 (síntese baseline vs principal).
import json, csv
...
def load_metrics(run_dir):
    ...
    return {
        "phase2_recall": p2.query("class == 'animal'").shape[0] /
                         max(1, p2.shape[0]),
...
def compute_macro_f1(df):
    # macro-F1 sobre as classes presentes nos datasets adotados
    # (Felis catus, Lynx rufus, Procyon lotor, Didelphis virginiana, Sciurus sp.)

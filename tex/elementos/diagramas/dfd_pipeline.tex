...
\begin{figure}[!htbp]
  ...
  \begin{tikzpicture}[
    ...
    \node[decision, right=of sn] (isfelis) {É \emph{Felis}\\\emph{catus}?};
    ...
    \node[proc, below=of reid] (out) {Relatórios\\AEX Gatosdoc2};
  ...
  \end{tikzpicture}
  \caption{Diagrama de Fluxo de Dados (DFD) arquitetural do pipeline de visão computacional. As quatro fases operam em cascata sobre o acervo bruto consolidado pelo ciclo \emph{offline-first}: o \modeloMD~descarta quadros vazios e dispara o mascaramento da classe ``pessoa'' antes de qualquer persistência identificável (conformidade com a LGPD, Seção~\ref{sec:pipeline-fase2}); o \modeloSN~descarta fauna não-alvo; e o estágio de reidentificação opera apenas sobre quadros confirmados como \emph{Felis catus}, contra o catálogo de indivíduos da colônia, alimentando os relatórios operacionais da AEX Gatosdoc2.}
